...
$tgl_ambil_pb = $_POST['tgl_ambil_pb']; // Ambil data nama dan masukkan ke variabel nama
//isset($_POST['"tgl_ambil"']) ? htmlspecialchars($_POST['"tgl_ambil"']) : "";

// query SQL untuk insert data

// $edit = mysqli_query($connect,"UPDATE tbl_daily SET tgl_ambil='$tgl_ambil' WHERE nis='$nis'");
$query = "UPDATE tbl_daily SET tgl_ambil_pb='$tgl_ambil_pb' WHERE id='$id'";
$hasil = mysqli_query($connect, $query)or die (mysqli_error($connect));
//$simpanstockopnameawal		= mysqli_query($connect,$insert)or die(mysqli_error($connect));


echo "Tanggal telah ditambah.";
echo '<META HTTP-EQUIV="REFRESH" CONTENT ="1; url=../index.php?halaman=penjadwalan3">';			
