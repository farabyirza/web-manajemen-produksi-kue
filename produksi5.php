<h5 style="text-align: right; font-family: Blackletter"><b><u>4</u></b></h5>
  ...
  <li class="active"><a href="index.php?halaman=produksi5">Jumat</a></li>
...
<!--   <li><a href="index.php?halaman=produksi8">Jumat</a></li> -->
    ...
    <?php
                        ...
                        // $query = mysqli_query($connect, "SELECT tbl_daily.nis, jadwal_produksi.id, tbl_daily.so FROM tbl_daily JOIN jadwal_produksi ON jadwal_produksi.id = tbl_daily.nis WHERE jadwal_produksi.so = tbl_daily.so")or die(mysqli_error($connect));
                         $query = mysqli_query($connect, "SELECT id,tgl_ambil_tb,tgl_produksi, sum(rata_ori), sum(rata_cho), sum(rata_che), sum(rata), sum(rata_kk), sum(rata_kv), sum(rata_tc), sum(rata_pb) FROM jadwal_pengambilan WHERE tgl_produksi LIKE '%(Jumat)%'")or die(mysqli_error($connect));
                                        ...
                                        {   
                                            ...
                                            while($data = mysqli_fetch_array($query)){  
                                                ...
                                                echo '<td><a href=index.php?halaman=input_produksi5><span class="glyphicon glyphicon-plus"></a></td>';
