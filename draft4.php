...
<!--   <li><a href="index.php?halaman=draft7">Minggu</a></li>  -->
  ...
  <li class="active"><a href="index.php?halaman=draft4">Kamis</a></li>
...
<!--         <th colspan="2">Action</th> -->
    ...
    <?php
                        ...
                        // $query = mysqli_query($connect, "SELECT tbl_daily.nis, jadwal_produksi.id, tbl_daily.so FROM tbl_daily JOIN jadwal_produksi ON jadwal_produksi.id = tbl_daily.nis WHERE jadwal_produksi.so = tbl_daily.so")or die(mysqli_error($connect));
                         $query = mysqli_query($connect, "SELECT id,tgl_ambil_tb,sum(rata_ori),sum(rata_cho),sum(rata_che), sum(rata), sum(rata_kk), sum(rata_kv), sum(rata_tc), sum(rata_pb) FROM jadwal_pengambilan WHERE tgl_ambil_tb LIKE '%Kamis%'")or die(mysqli_error($connect));
